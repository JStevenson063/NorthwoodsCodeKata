...
`);
  ...
  getBedTime(bedTimeString) {
    ...
    if (
      `${bedTimeDate.getHours()}:${bedTimeDate.getMinutes()}` < `${CliFunctions.EARLIEST_START_TIME.getHours()}:${CliFunctions.EARLIEST_START_TIME.getMinutes()}` ||
  ...
  calculatePayment(startTimeDate, bedTimeDate, endTimeDate) {
    ...
    let totalPayment = 0;
    const bedTimeMinusStartTime = Math.floor((((bedTimeDate.getTime() - startTimeDate.getTime()) / 1000) / (60 * 60)));
    const midnightMinusBedTime = Math.floor((((midnightTest.getTime() - bedTimeDate.getTime()) / 1000) / (60 * 60)));
    const endTimeMinusMidnight = Math.floor((((endTimeDate.getTime() - midnightTest.getTime()) / 1000) / (60 * 60)));

    if (Math.abs(bedTimeMinusStartTime * CliFunctions.START_TIME_TO_BED_TIME_PAY) > 0) {
      totalPayment += Math.abs(bedTimeMinusStartTime * CliFunctions.START_TIME_TO_BED_TIME_PAY);
    }
      totalPayment += Math.abs(midnightMinusBedTime * CliFunctions.BED_TIME_TO_MIDNIGHT_PAY);
